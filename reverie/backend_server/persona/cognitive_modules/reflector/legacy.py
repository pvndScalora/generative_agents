...
class LegacyReflector(AbstractReflector):
    ...
    def reflect(self):
        ...
        if self.persona.scratch.chatting_end_time: 
            if self.persona.scratch.curr_time + datetime.timedelta(0,10) == self.persona.scratch.chatting_end_time: 
                all_utt = ""
                if self.persona.scratch.chat: 
                    for row in self.persona.scratch.chat:  
                        all_utt += f"{row[0]}: {row[1]}\n"

                evidence = [self.persona.a_mem.get_last_chat(self.persona.scratch.chatting_with).node_id]

                planning_thought = self._generate_planning_thought_on_convo(all_utt)
                planning_thought = f"For {self.persona.scratch.name}'s planning: {planning_thought}"
                ...
                memo_thought = f"{self.persona.scratch.name} {memo_thought}"
    ...
    def _reflection_trigger(self): 
        print (self.persona.scratch.name, "persona.scratch.importance_trigger_curr::", self.persona.scratch.importance_trigger_curr)
        print (self.persona.scratch.importance_trigger_max)

        if (self.persona.scratch.importance_trigger_curr <= 0 and 
            [] != self.persona.a_mem.seq_event + self.persona.a_mem.seq_thought): 
            return True 
        return False

    def _reset_reflection_counter(self): 
        persona_imt_max = self.persona.scratch.importance_trigger_max
        self.persona.scratch.importance_trigger_curr = persona_imt_max
    ...
    def _generate_focal_points(self, n=3): 
        ...
        statements = ""
        for node in nodes[-1*self.persona.scratch.importance_ele_n:]: 
            statements += node.embedding_key + "\n"

        return run_gpt_prompt_focal_pt(self.persona, statements, n)[0]
    ...
    def _generate_insights_and_evidence(self, nodes, n=5): 
        ...
        ret = run_gpt_prompt_insight_and_guidance(self.persona, statements, n)[0]
    ...
    def _generate_planning_thought_on_convo(self, all_utt):
        ...
        return run_gpt_prompt_planning_thought_on_convo(self.persona, all_utt)[0]

    def _generate_memo_on_convo(self, all_utt):
        logging.debug("GNS FUNCTION: <generate_memo_on_convo>")
        return run_gpt_prompt_memo_on_convo(self.persona, all_utt)[0]
